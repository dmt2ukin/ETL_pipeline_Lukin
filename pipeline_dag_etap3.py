...
dag = DAG(
    ...
    catchup=False,
    description='ETL pipeline for breast cancer prediction with model evaluation and saving'
)

DATA_PATH = 'data/breast_cancer_dataset.csv'
MODEL_PATH = 'results/model.joblib'
METRICS_PATH = 'results/metrics.json'
PREPROCESSED_PATH = 'results/preprocessed.pkl'
METRICS_TMP_PATH = 'results/metrics.pkl'
...
def task_preprocess(**context):
    ...
    joblib.dump((X, y, scaler), PREPROCESSED_PATH)

def task_train_model():
    X, y, _ = joblib.load(PREPROCESSED_PATH)
    model = train(X, y)
...
def task_evaluate_model():
    ...
    joblib.dump(metrics, METRICS_TMP_PATH)

def task_save_results():
    metrics = joblib.load(METRICS_TMP_PATH)
